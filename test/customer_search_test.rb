...
class CustomerSearchTest < Minitest::Test
  ...
  def test_matching_customers_are_returned_and_sorted_by_attribute
    ...
    assert all_customers, worldwide_customers
    ...
    assert nearby_customers, customers
  end
  ...
  def test_only_name_and_user_id_are_returned_per_customers
    ...
    assert worldwide_customers.all? { |c| c.keys.all? { |k| CustomerSearch::DESIRED_OUTPUT_CUSTOM_ATTRS.include?(k) } }
  end
end
